#!/usr/bin/env python
''' Testing Worker Provision Handler '''
...
class TestingWorkerProvisionHandler(BaseHandler):
    """
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            # Step One - Query Automation Database for Testing Worker Provisioning Data
            if self.provision_provider:
                ...
                # Step Two - Trigger Testing Worker from Testing Worker Provisioning data
                result = self.provision_provider.trigger_testing_worker(stack_name=stack_name, automation_data=automation_data, tenant_name=tenant_name)
                if result['result']:
                    msg = 'Successfully kicked off testing worker for stack: %s at tenant name: %s' % (stack_name, tenant_name,)
                    body = {"results": msg}
                else:
                    msg = 'Failed to kick off testing worker for stack: %s at tenant name: %s' % (stack_name, tenant_name,)
                    body = {"results": msg}
            else:
